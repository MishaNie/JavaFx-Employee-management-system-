...
public class ManagerController implements Initializable{
    ...
    private void onIdEnterPress(KeyCode code)
    {
        if(textID.isFocused() && code.getCode() == KeyCode.ENTER.getCode() && textID.getText().length() > 0)
        {
            ...
            {
                showErrorBox("numero Employé", "Le numéro d'employé est invalide",
                    "Merci d'entrer uniquement des nombres entiers positif");
            }
        }
        else
        {
    ...
    private void onExperienceEnterPress(KeyCode code)
    {
        ...
        {//validate if experience is integer
            ...
            {
                showErrorBox("Expérience", "L'expérience de l'employé est invalide",
                    "Merci d'entrer uniquement des nombres entiers positifs");
            }
        }
        else
        {
